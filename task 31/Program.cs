...
int[] GetSumPositivMefativeElem(int[] arr)
{
    int sumPositive = 0;
    int sumNegative = 0;
    for (int i = 0; i < arr.Length; i++)
...
int [] getSumPositivMefativeElem = GetSumPositivMefativeElem(array);

Console.WriteLine($"Сумма положительных элементов => {getSumPositivMefativeElem[0]}");
Console.WriteLine($"Сумма отрицательных элементов => {getSumPositivMefativeElem[1]}");
